...
const TOUR_PACKAGES = [
  ...
  },
  {
    ...
    highlights: [
      "Sheikh Zayed Grand Mosque VIP Entry",
  ...
  },
  {
    ...
    highlights: [
      "Fast-Track Skip-The-Line Priority Entry",
  ...
  },
  {
    ...
    highlights: [
      ...
      "Commemorative VIP Flight Certificate",
    ],
    badge: "Exclusive Aerial",
  },
  {
    id: "yacht-experience",
...
export const PackagesSection: React.FC = () => {
  ...
  const getWhatsAppPackageLink = (title: string) => {
    const text = `Hello SK Tourism, I would like to inquire about booking the featured package: ${title}. Please share availability and details.`;
    return `https://wa.me/${BUSINESS_INFO.whatsappNumber}?text=${encodeURIComponent(text)}`;
  };

  return (
    <section id="packages" className="py-20 bg-[#F8F9FA] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-center max-w-3xl mx-auto mb-12 space-y-3">
          <Badge variant="gold" className="px-4 py-1.5 font-bold tracking-widest">
            Curated VIP Itineraries
          </Badge>
          <h2 className="text-3xl sm:text-5xl font-serif font-extrabold text-[#1F2937] tracking-tight">
            Featured <span className="gold-text-gradient">Tour Packages</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {TOUR_PACKAGES.map((pkg, idx) => (
            <motion.div
              ...
              className="glass-card rounded-2xl overflow-hidden border border-gray-200 hover:border-[#D4AF37] transition-all duration-500 hover:shadow-2xl flex flex-col justify-between group"
            >
              {/* Image */}
              <div className="relative h-56 w-full overflow-hidden">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#0A365C]/80 via-transparent to-transparent opacity-60" />
              ...
              <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                <div>
                  <h3 className="font-serif text-xl font-bold text-[#1F2937] group-hover:text-[#0F4C81] transition-colors mb-3">
                    {pkg.title}
                  </h3>

                  <div className="space-y-2">
                    {pkg.highlights.map((h, hIdx) => (
                      <div key={hIdx} className="flex items-center space-x-2 text-xs text-gray-700">
                ...
                <div className="pt-4 border-t border-gray-100 flex items-center justify-between space-x-2">
